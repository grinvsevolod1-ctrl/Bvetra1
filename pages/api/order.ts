...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (body.message) {
    parsed = await parseWithOpenAI(body.message);
  }

  // fallback: map fields manually
  if (!parsed) {
    parsed = {
      name: body.name || null,
  ...
  try {
    if (bitrixWebhook) {
      ...
      if (parsed.phone) {
        leadFields.PHONE = [{ VALUE: parsed.phone, VALUE_TYPE: 'WORK' }];
      }

      if (parsed.email) {
        leadFields.EMAIL = [{ VALUE: parsed.email, VALUE_TYPE: 'WORK' }];
      }
      ...
      if (process.env.BITRIX_DEAL_WEBHOOK) {
        const dealFields: any = {
          TITLE: 'Новый трансфер/аренда',
          CONTACT_ID: null,
          OPPORTUNITY: 0
        };

        if (parsed.notes) {
          dealFields.COMMENTS = parsed.notes;
        }
